...
void LPS::writeReg(int reg, byte value)
{
  ...
  HAL_I2C_Mem_Write(&hi2c1, address , reg , 1 , &byte , 1 , HAL_MAX_DELAY);
...
byte LPS::readReg(int reg)
{
  ...
  HAL_I2C_Mem_Read( &hi2c1 , address , reg , 1 , &value , 1 , HAL_MAX_DELAY);
...
{
	uint8_t what = WHO_AM_I;

	HAL_I2C_Master_Transmit(&hi2c1 , address , &what , 1 , 200);
